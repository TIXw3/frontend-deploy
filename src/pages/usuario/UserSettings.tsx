...
const UserSettings: React.FC<UserSettingsProps> = ({
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      const response = await api.put("/usuarios/tornar-organizador");
      const { usuario: usuarioAtualizado, token: novoToken } = response.data.data;
      ...
      alert("❌ Erro ao se tornar organizador. Tente novamente.");
    } finally {
  ...
  const handleConfirm = () => {
    ...
    try {
      ...
      setTimeout(() => setSaveStatus("idle"), 3000); // Limpar status após 3 segundos
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            <div className="flex flex-col items-center justify-center h-full gap-4">
              ...
              {saveStatus === "success" && (
                <p className="text-green-500 text-sm">Configurações salvas com sucesso!</p>
              )}
              <button
                onClick={handleSave}
